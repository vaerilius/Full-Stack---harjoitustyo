                ...
                {props.job.questions.map(q =>
                  <li key={q._id} className="list-group-item ">
                    ...
                    <div className="d-flex justify-content-between border-top border-info">
                      <Link to={!q.questioner.jobProvider ? `/candidates/${q.questioner.id}` : `/providers/${q.questioner.id}`}>
                      < h5>{q.questioner.name}</h5>
                      <img src={q.questioner.picture}
                        className="my-auto rounded-circle" style={{ height: '2rem' }} alt={q.questioner.name} />
                      </Link>
